...
for i in range(round):
    ...
    for time in range(0, sim_time):
        ...
        for i in range(1):
            a = random.randint(0, int(node_num * Gp.com_node_rate/2))
            node_list[com_node_list[a][0]].generate_request(com_node_list[a][1], controller, 1024, t.time())
            controller.resolve_request(node_list)
            node_list[com_node_list[a][0]].forward_pkt_to_nbr(node_list, controller)
            print('\n')
...
print(std2/round)
print('\ndelivery ratio:\n')
print((sim_time*round*1-Gp.fail_time)/sim_time/round/1)

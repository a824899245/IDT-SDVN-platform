...
class Node:
    ...
    # 发送错误请求
    def generate_error(self,  source_id,  des_id, controller, seq, node_list, time, next_hop):
        ...
        # 时延处理
        # 控制器增加此错误路由
        controller.flow_error_list.append(Pkt.FlowError(source_id,  des_id,  self.node_id, 1, seq, seq, time, next_hop))
        self.pkt_seq = self.pkt_seq + 1
    ...
    def receive_flow(self, flow_reply):
        ...
        for key, node_id in enumerate(flow_reply.route):
            if node_id == self.node_id:
                self.routing_table.append(Pkt.RoutingTable(flow_reply.source_id, flow_reply.des_id, flow_reply.route[key+1], 0, flow_reply.node_id, flow_reply.seq, 0))
                return
    ...
    #         self.routing_table.append(Pkt.RoutingTable(geo_flow_reply.source_id, self.node_id, self.node_id, 0, geo_flow_reply.node_id, geo_flow_reply.seq, 0.3))
    #         return
    #
    #     # 如果此条目已存在，更新信息
    ...
    #         self.routing_table.append(Pkt.RoutingTable(geo_flow_reply.source_id, 9999, i, 0, geo_flow_reply.node_id, geo_flow_reply.seq, 0.3))
        ...
        for pkt in self.data_pkt_list[::-1]:
            for table in self.routing_table[::-1]:
                if self.cache < 0:
                    ...
                    self.generate_error(pkt.source_id, pkt.des_id, self.controller, pkt.seq, node_list, time.time(), table.next_hop_id)
                # 如果路由表条目与分组相符
                if pkt.seq == table.seq and pkt.node_id == table.node_id:
                    ...
                    d = pow(node_list[table.next_hop_id].position[0] - self.position[0],2) + \
                        pow(node_list[table.next_hop_id].position[1] - self.position[1],2)
                    # 转发成功与否判断
                    if d < pow(Gp.com_dis, 2):
                        # 时延计算
                        if pr.ratio(math.sqrt(d)) == 1:
                        # 下一跳节点收分组
                            node_list[table.next_hop_id].receive_pkt(pkt, node_list, controller, self)
                            # 改变路由条目与分组状态以删除
    ...
    #             # 如果路由表条目与分组相符
    #             if pkt.seq == table.seq and pkt.node_id == table.node_id:
    #                 d = pow(node_list[table.next_hop_id].position[0] - self.position[0], 2+ self.velocity[0]*0.03) + \
    #                     pow(node_list[table.next_hop_id].position[1] - self.position[1], 2+ self.velocity[1]*0.03)#(table.dur+pkt.delay) )
    #                 # 转发成功与否判断
    #                 if d < pow(Gp.com_dis, 2):
    #                     # 时延计算
    #                     # 下一跳节点收分组
    ...
    #                     # 路由发生错误，回调路由条目与分组状态，发送错误请求
    #                     print('node %3d to node %3d 距离超过' % (self.node_id, node_list[table.next_hop_id].node_id))
    #                     self.generate_error(pkt.source_id, table.next_hop_id, self.controller, pkt.seq, node_list, time.time())
    #                     break
    ...
    # 接受转发来的分组
    def receive_pkt(self, data_pkt, node_list, controller, last_hop):
        # 到达终点 转发成功
        data_pkt.delay += 0.03
        last_hop.cache -= 1
    ...
    #                 Gp.sum = Gp.sum  + data_pkt.delay
    #                 Gp.record.append(+ data_pkt.delay)
    #                 # 时延总计算
    #                 # 丢包率计算
    #                 return
    #         # 自己添加改包 并继续转发

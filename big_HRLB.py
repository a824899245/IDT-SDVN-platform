...
y = []
junction_grid = []
grid_sub = [[] for i in range(169)]
grid_num = [[] for i in range(169)]
grid_trans = [[0 for i in range(169)] for i in range(169)]
grid_adj = [[0 for i in range(169)] for i in range(169)]
max_n = 1
max_t = 1
g_s = []
ss = []
...
def grid_intiall():
    for i in range(169):
        ...
        if i % 13 == 0 and i != 0:
            grid_adj[i][i - 1] = 0
        if i % 13 == 12 and i != 168:
            grid_adj[i][i + 1] = 0
    for key, i in enumerate(bji.junction_position):
        if i[0] != -1:
            x.append(i[0])
            y.append(i[1])
            junction_grid.append(int(i[0] / 500) + int(i[1] / 500) * 13)
            grid_sub[int(i[0] / 500) + int(i[1] / 500) * 13].append(key)
...
def cal_seg(s_inter, d_inter, node_list):
    veh_list = []
    result = bji.junction_distance[s_inter][d_inter]
    result += (len(bji.junction_vehicle[s_inter]) + len(bji.junction_vehicle[d_inter]))
    for i in bji.junction_vehicle[s_inter]:
        veh_list.append(i)
    for i in bji.junction_vehicle[d_inter]:
        veh_list.append(i)
    dis = 0
    for i in veh_list:
        for j in veh_list:
    ...
    for i in range(len(g_s)-1):
        ...
        for s_inter in grid_sub[g_s[i]]:
            for d_inter in grid_sub[g_s[i+1]]:
                if bji.adj_martix[s_inter][d_inter] != 0:
...
def poss_veh_gen():
    for i in range(len(seg_seq)-1):
        for j in bji.junction_vehicle[seg_seq[i]]:
            vehicle_seq.append(i)
        for k in bji.junction_vehicle[seg_seq[i+1]]:

...
alpha = 0.5
alpha_martix = [[0.5 for i in range(80)] for i in range(80)]
beta = 1 - alpha
beta_martix = [[(1-alpha) for i in range(80)] for i in range(80)]
k_i = 1
k_i_martix = [[1 for i in range(80)] for i in range(80)]
        ...
        for j in range(80):
            if ji.adj_martix[i][j] != 0:
                ...
                if a == 0:
                    ...
                    r[i][j] = alpha_martix[i][j]*(k_i_martix[i][j]*a+(2-k_i_martix[i][j])*b) + (1-alpha_martix[i][j])*c
    for i in range(2, 9):
        r[20 + i][30 + i] = 0
        r[30 + i][40 + i] = 0
    r[31][32] = 0

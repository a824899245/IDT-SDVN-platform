        ...
        for j in range(80):
            if ji.adj_martix[i][j] != 0:
                ...
                if a == 0:
                    ...
                    r[i][j] = alpha*(k_i*a+(2-k_i)*b) + beta*c
    for i in range(2, 9):
        r[20 + i][30 + i] = 0
        r[30 + i][40 + i] = 0
    r[31][32] = 0
...
def hidden_seq_generate(reward, source, des):
    ...
    for i in range(80):
        for j in range(80):
            if reward[i][j] != 0 and i != j:
               g.add_edge(i, j, weight=1000-reward[i][j])
    try:
        nx.shortest_path(g, source=source, target=des)
    except nx.NetworkXNoPath as err1:
        a = None
    except nx.NodeNotFound as err2:
        a = None
    else:
...
def hidden_to_obverse(source_vehicle, des_vehicle, node_list, hidden_seq, error_vehicle):
    ...
    for i in range(len(hidden_seq)-1):
        if i == 0:
            for nodei in ji.junction_vehicle[hidden_seq[i + 1]]:
                ...
                if d < pow(Gp.com_dis, 2) and (source_vehicle != error_vehicle and node != error_vehicle):
            ...
            if i == len(hidden_seq)-1:
                for nodei in ji.junction_vehicle[hidden_seq[i]]:
                    ...
                    if d < pow(Gp.com_dis, 2) and (des_vehicle != error_vehicle and node != error_vehicle):
                ...
                for s_nodei in ji.junction_vehicle[hidden_seq[i]]:
                    ...
                    for d_nodei in ji.junction_vehicle[hidden_seq[i+1]]:
                        ...
                        if d < pow(Gp.com_dis, 2) and (s_node != error_vehicle and d_node != error_vehicle):
